...
class DispOffsetCombiner(StereoCombiner):
    """
    Calculate array-event disp prediction as (weighted) mean of telescope-wise predictions
    in a NominalFrame centered on the array pointing
    """
    ...
    def predict_table(self, mono_predictions: Table) -> Table:
        ...
        if len(valid_predictions) > 0:
            ...
            fov_lon_mean = _weighted_mean_ufunc(
                fov_lons.to_value(u.deg),
                weights,
                n_array_events,
                indices[valid],
            )
            fov_lat_mean = _weighted_mean_ufunc(
